...
export default makeScene2D(function* (view) {
    ...
    yield* console().terminal().line("/workspaces/fpga-router/src/infrastructure/client/NanomsgProducerClient.cpp:13:5:", colorRed);
    yield* console().terminal().line("error: 'm_socket' should be initialized in a member initializer of the constructor", colorRed);
    yield* console().terminal().line("[cppcoreguidelines-prefer-member-initializer,-warnings-as-errors]", colorRed);
    yield* console().terminal().line("NanomsgProducerClient::NanomsgProducerClient(const std::string& url)", colorRed);
    yield* console().terminal().prompt();

    yield* beginSlide("FIX");

    yield* all(
        code().selection(word(0, 69, 37), animationTime),
