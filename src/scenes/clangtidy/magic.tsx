...
export default makeScene2D(function* (view) {
    ...
    yield* code().selection(word(7, 29, 3), animationTime);
    ...
    yield* console().terminal().line("/workspaces/fpga-router/src/infrastructure/client/NanomsgProducerClient.cpp:19:30:", colorRed);
    yield* console().terminal().line("error: 100 is a magic number; consider replacing it with a named constant", colorRed);
    yield* console().terminal().line("[cppcoreguidelines-avoid-magic-numbers,readability-magic-numbers,-warnings-as-errors]", colorRed);
    yield* console().terminal().line("int timeout = 100;", colorRed);
    yield* console().terminal().prompt();

    yield* beginSlide("FIX");

    yield* all(
        code().selection(lines(0, 3), animationTime),
